...
def document_to_proto(document_root, plaintext):
    ...
    for sentence_tag in document.find('sentences'):
        ...
        for token in sentence_tag.find('tokens'):
            ...
            output_token.startOffset = token_start
            output_token.endOffset = token_end
    ...
    for coreference_tag in (document.find('coreference') or []):
        ...
        for mention_tag in coreference_tag.findall('mention'):
            ...
            mention_end_id = int(mention_tag.find('end').text) - 1
            ...
            output_mention.sentenceIndex = sentenceid
            output_mention.startWordIndex = mention_start_id
            output_mention.endWordIndex = mention_end_id
...
for root, subdirs, files in os.walk(args.plaintexts_dir):
    for filename in files:
        ...
        output_document = document_to_proto(document_root, plaintext)
